...
演示拓扑：`server`（启动时 `[demo]` 引导种入 demo-node）→ `agent` + `target`，`curl` 经 HTTP 隧道打到 `target`。
...
- **凭据**：server 启动时经 `[demo]` 配置段种入 `demo-node`，凭据以 SHA-256 哈希入库（明文仅存在于
  `config/agent.toml` 与 `config/server.toml` 的 `[demo].token`，二者须一致）。
...
- 用 Web 后台 / API 签发随机 token，替换 `rstunnel-demo-token`。
